...
export class SimpleWeatherNewsCdkProjectStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // GET /{cityId} エンドポイント（第2段階で追加）
    const getCityIntegration = new integrations.HttpLambdaIntegration('GetCityWeatherIntegration', getCityWeatherPublicFunction);
    httpApi.addRoutes({
      path: '/{cityId}',
      methods: [apigatewayv2.HttpMethod.GET],
      integration: getCityIntegration,
    });
  }
}